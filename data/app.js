...
function startVectorMonitor(){
  ...
  let modeIndex = 0;
  let t = 0;
  let phosphor = document.createElement('canvas');
  ...
  function draw(){
    ...
    // redraw grid faintly into phosphor
    drawGrid(pctx);

    // draw new bright vector into phosphor
    ...
    // bright current frame overlay
    ctx.globalAlpha = 1;
    drawCurrentFrame(ctx);
    ctx.shadowBlur = 0;

    // subtle noise bars / scan shimmer
  ...
  const prev = q('#vecPrev'), next = q('#vecNext');
...
const batteryProfiles={
  ...
  liion1s:{good:2.80,low:2.55,empty:2.20,label:'Li-Ion limited-to-3V window preset active.'}
};

function setText(sel,t){const el=q(sel); if(el) el.textContent=t;}
function num(v){const n=Number(v);return Number.isFinite(n)?n:null;}
function fmt(n,u=''){return `${Number(n).toLocaleString('sv-SE',{maximumFractionDigits:6})}${u?` ${u}`:''}`;}
...
function batteryZone(v){
  ...
  const p=batteryProfiles[(sel && sel.value) || 'aa'] || batteryProfiles.aa;
  setText('#batteryHint', p.label);
  if(!Number.isFinite(v)) return '-';
  if(v >= p.good) return 'GOOD';
  if(v >= p.low) return 'MID';
  if(v >= p.empty) return 'LOW';
  return 'DEAD';
}
...
function updateUiFromState(d){
  if(d.mv!==undefined){
    ...
    if(Number.isFinite(mv)){
      ...
      setText('#voltageLbl', vv.toFixed(3));
      setText('#zoneLbl', batteryZone(vv));
    }
  }
  if(d.adc!==undefined) setText('#adcLbl', `${d.adc} / 1023`);
  if(d.rssi!==undefined) setText('#rssiLbl', `${d.rssi}`);
  if(d.ip) setText('#ipLbl', d.ip);
  if(d.mode) setText('#modeLbl', d.mode);
  setText('#statusLbl', 'HTTP OK');
}
...
function syncLawMode(){
  ...
  q('#lawA').placeholder = cfg.a;
  q('#lawB').placeholder = cfg.b;
}

function bindCalc(){
  syncLawMode();
  q('#lawMode').onchange = syncLawMode;

  q('#btnCalcLaw').onclick=()=>{
    const A=num(q('#lawA').value), B=num(q('#lawB').value);
    if(A===null || B===null){ setText('#lawOut','Enter valid values in both fields.'); pulse(q('#btnCalcLaw')); return; }
    let out='-';
    switch(q('#lawMode').value){
      case 'voltage_from_ir': out=`Voltage = ${fmt(A*B,'V')}`; break;
      case 'current_from_ur': out = B!==0 ? `Current = ${fmt(A/B,'A')}` : 'Division by zero.'; break;
      case 'res_from_ui': out = B!==0 ? `Resistance = ${fmt(A/B,'Ohm')}` : 'Division by zero.'; break;
      case 'power_from_ui': out=`Power = ${fmt(A*B,'W')}`; break;
      case 'current_from_pu': out = B!==0 ? `Current = ${fmt(A/B,'A')}` : 'Division by zero.'; break;
      case 'voltage_from_pi': out = B!==0 ? `Voltage = ${fmt(A/B,'V')}` : 'Division by zero.'; break;
    }
    setText('#lawOut', out);
    pulse(q('#btnCalcLaw'));
  };

  q('#btnCalcLawClear').onclick=()=>{
    q('#lawA').value=''; q('#lawB').value=''; setText('#lawOut','Cleared.'); pulse(q('#btnCalcLawClear'));
  };

  const resList=q('#resistorList');
  function renumber(){ qa('#resistorList .resInput').forEach((inp,i)=>inp.placeholder=`R${i+1} Ohm`); }
  function bindRemovers(){
    qa('.resRemove').forEach(btn=>btn.onclick=()=>{
      const rows=qa('#resistorList .resRow');
      if(rows.length<=2){ btn.parentElement.querySelector('.resInput').value=''; return; }
      btn.parentElement.remove(); renumber();
    });
  }
  q('#btnAddResistor').onclick=()=>{
    const row=document.createElement('div');
    row.className='resRow';
    row.innerHTML='<input class="field resInput" type="number" step="any" placeholder="R Ohm"><button class="btn ghost resRemove" type="button">x</button>';
    resList.appendChild(row); bindRemovers(); renumber(); pulse(q('#btnAddResistor'));
  };
  q('#btnCalcResistors').onclick=()=>{
    const vals=qa('#resistorList .resInput').map(el=>num(el.value)).filter(v=>v!==null&&v>0);
    if(vals.length<2){ setText('#resOut','Enter at least two positive resistor values.'); pulse(q('#btnCalcResistors')); return; }
    if(q('#resMode').value==='series') setText('#resOut', `Series total = ${fmt(vals.reduce((a,b)=>a+b,0),'Ohm')}`);
    else {
      const inv=vals.reduce((a,b)=>a+1/b,0);
      setText('#resOut', `Parallel total = ${fmt(1/inv,'Ohm')}`);
    }
    pulse(q('#btnCalcResistors'));
  };
  q('#btnCalcResistorsClear').onclick=()=>{
    resList.innerHTML='<div class="resRow"><input class="field resInput" type="number" step="any" placeholder="R1 Ohm"><button class="btn ghost resRemove" type="button">x</button></div><div class="resRow"><input class="field resInput" type="number" step="any" placeholder="R2 Ohm"><button class="btn ghost resRemove" type="button">x</button></div>';
    bindRemovers(); renumber(); setText('#resOut','Cleared.'); pulse(q('#btnCalcResistorsClear'));
  };
  bindRemovers(); renumber();
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  setText('#zoneLbl','-');
  q('#batterySel').addEventListener('change',()=>{
